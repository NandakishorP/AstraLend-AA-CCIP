...
const POOL_ABI = [
  ...
  "function borrowLoan(uint64,uint256,uint256) payable",
...
async function main() {
  ...
  // Foreclosure permanently removes tokens from the holder, so a second run
  // starts short. Top back up to the target so the scenario is rerunnable
  // against a persistent node.
  ...
  if (opening < TARGET) {
    ...
    await send(
      ...
      "mint",
      [holder.address, TARGET - opening]
    );
    await hub.send("anvil_stopImpersonatingAccount", [issuerOwner]);
    note(`topped up to ${tk(await token.balanceOf(holder.address))} ${symbol} for a repeatable run`);
  }
  ...
  const expected = await issuer.quoteRedemption(pledge);
  ...
  await send(trusteeIssuer, "redeem", [pledge]);
